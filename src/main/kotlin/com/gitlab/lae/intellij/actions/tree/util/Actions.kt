...
fun setEnabledModalContext(event: AnActionEvent, action: AnAction) {
  if (event.presentation.isEnabled && isModalContext(event) && !action.isEnabledInModalContext) {
    event.presentation.isEnabled = false
  }
}

private fun isModalContext(e: AnActionEvent): Boolean = e.getData(IS_MODAL_CONTEXT) ?: false
