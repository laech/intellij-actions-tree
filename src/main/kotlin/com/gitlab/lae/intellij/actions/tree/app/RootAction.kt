...
data class RootAction(
  ...
  companion object {
    ...
    fun merge(
      ...
      val byActions =
          byKeys.entries.groupBy { it.value }.mapValues { it.value.map { e -> e.key } } +
              (if (noKeys.isEmpty()) emptyMap() else mapOf(noKeys to emptyList()))
      ...
      return generateSequence(0, Int::inc)
          ...
          .map { (i, nodes, keys) ->
             ...
             */
            val id =
                if (nodes.size == 1 && nodes[0].items.isNotEmpty()) {
                  nodes[0].id
                } else {
                  "ActionsTree.Root.$i"
                }
            ...
            RootAction(id, keys, nodes.reversed()).apply {
              ...
              this.isEnabledInModalContext =
                  this.actions.any { it.toAction(actionManager).isEnabledInModalContext }
            }
          }
          .toList()
    }
